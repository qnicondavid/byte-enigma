...
class RekeyEquivalenceTest {
    ...
    void rekeyMatchesFreshConstructionAcrossSeedsAndRotorCounts() {
        ...
        for (int rotorCount = 1; rotorCount <= 6; rotorCount++) {
            ...
            for (int seed : seeds()) {
                reused.rekey(seed);
                byte[] fresh = new ByteEnigma(seed, rotorCount).transform(input);
    ...
    void repeatedRekeyToSameSeedIsStable() {
        ...
        int rotorCount = 4;
        int seed = -12345;
        ByteEnigma fresh = new ByteEnigma(seed, rotorCount);
        byte[] expected = fresh.transform(input);
        ByteEnigma reused = new ByteEnigma(999, rotorCount);
